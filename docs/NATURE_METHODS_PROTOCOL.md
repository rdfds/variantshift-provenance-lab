...
The current pilot is a negative development result. VariantShift risk–coverage AUC is 0.00407,
versus 0.00026 for the strongest development comparator, and the nested-bootstrap improvement
estimate is −0.00381 (95% interval −0.01491 to 0.00000). Observed cross-fitted lower-bound coverage
is 98.97%, indicating over-conservative calibration. The method must improve on development data
before it is frozen for confirmation.
...
materially different input/model families and shared coverage of at least 95% on 300 targets.

## Frozen Transportability Score

The candidate implementation is a histogram gradient-boosting regressor over task and model
descriptors. It predicts selection gain for each task–model pair. A one-sided group-conformal lower
bound is computed from worst residuals within held-out calibration families. The model with the
largest lower bound is selected only when that bound exceeds zero.
